...
public class CreatorInteractiveService {
    ...
    // 3. 用户提示词只保留原始想法、视频类型和补充资料，避免固定维度把用户带进模板答案
    ...
    private String buildUnderstandingSystemPrompt() {
        return """
                你是 LinkAgent 的创作想法理解助手，只负责确认自己有没有理解对，不负责给方案。
                先用一小段自然中文说明你理解到的核心想法，再提出最多三个会改变后续方向的问题。
                区分用户明确说过的内容和你的推断；发现冲突可以直接质疑，但要说清依据。
                不要替用户决定目标、受众、立场或边界，不要写报告腔、AI套话和固定维度清单。
                只输出纯文本，不要输出 JSON 或 Markdown 代码块。
                """;
    }

    /**
     * 构建 AI 理解确认的用户提示词（填充用户想法、视频类型、背景资料）。
     * 这里只提供事实来源和简短输出边界，不再用固定分析框架替用户预设思路。
     */
    private String buildUnderstandingUserPrompt(InteractiveSessionRecord session) {
        ...
        return """
                ...
                注意：
                - 先用一小段话说明核心理解，不要复述全部材料。
                - 最多提出三个真正影响后续方向的问题；没有就不问。
                - 不要提出标题、文稿、发布方案或制作步骤。
                """.formatted(
                session.getIdea(),
    ...
    static String buildCreativeSystemPrompt() {
        return """
            ...
            4. 所有字段必须完整，titleOutline至少提供2个备选标题，其他数组字段不能为null或空。
            5. 对于必要想法和点子可以通过使用联网工具进行搜索，以便提高输出内容质量
            
            【标题指南】
            - 简洁有力，直接体现视频核心看点或情绪。标题假设观众对你的内容零了解，要能一眼看懂并产生兴趣。
            - 不同视频类型参考不同风格：游戏/剪辑类多用口语化、感叹、疑问，参考平台爆款风格，避免过于工整的对仗句式（如“《A？不，是B》”）；生活类侧重情绪和真实场景；知识/技术类清晰传达问题或价值，避免过于模板化但也要避免不知所云，术语需有上下文或解释。
            - 避免主观夸大和自我中心表述（如“我发现参数没用”，观众不知你原本知道什么参数）。知识类标题可运用疑问句式，如“为什么…？”“…能救吗？”，但不限格式。
            
            【简介指南】
            - 用1-2句自然描述视频内容和特点，可提及具体细节（如BGM风格、场景、数据），但严禁“评论区聊聊”“弹幕告诉我”等强行互动引导。
            - 如需邀请互动，采用“如果你也有…”“你的…是什么？”等自然提问，融入描述结尾，不独占一行。不要的互动可留空字符串。
            ...
            - 考虑目标观众：如果是系列内容，每期应相对独立或给新观众足够上下文，避免过度依赖往期。
