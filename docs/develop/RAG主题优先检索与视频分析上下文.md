...
先 RAG 检索主题中块
-> 聚合命中的 videoId
-> 命中视频先按 quality_score DESC, raw_quality_score DESC 形成 top20 候选
...
- 最多保留 20 个候选视频。
- 回查 MySQL 时按 `category` / `tier` 二次过滤，再按 `quality_score DESC, raw_quality_score DESC, id DESC` 形成候选池；低样本分区没有可靠相对分时，仍可用单视频原始分兜底排序。
...
- rerank 只调整 top20 候选顺序，不改变候选集合；关闭、未配置 key 或调用失败时返回 `reranked=false`，继续按质量信号展示。
