...
它是项目内固定的 BV 拉取脚本，会被前端通过后端接口受控执行；脚本不使用 Cookie，不处理登录态，不绕过平台风控。
...
| 参数 | 说明 |
|---|---|
| `--output-dir` | 输出目录，默认项目内 `export/bilibili_feedback` |
...
当前后端评论弹幕分析接口已经支持两种入口：

1. 用户手动上传脚本输出文件。
2. 前端填写 BV 后，由后端执行本脚本并自动导入。

脚本产物仍然是任务级样例数据，后续如果要扩展更多导入格式，需要单独设计入参校验、大小限制和错误处理。
