...
LinkAgent 是开源、个人自托管的创作者工作台。默认一套部署由一位创作者使用，核心场景是拉取仓库后在自己的环境中运行，而不是提供公开注册和多人协作的 SaaS 服务。

公网地址只用于作品演示和联调，不代表对外提供正式服务。后续即使补充 userId 或数据隔离，也只为避免不同本地或演示使用场景的数据混淆，不应被理解为多租户产品规划。

因此，功能设计优先保证本地/自托管可用性和创作者工作流完整性。基础保护仍然保留，例如不提交密钥、不把私有媒体设为公共读；但不因演示地址存在就默认引入完整账号体系、网关、RBAC、配额或企业级安全校验。
